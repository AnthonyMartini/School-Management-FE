      ...
      tooltip: (
        <div className="space-y-1">
          ...
          <p className="text-[9px] text-gray-400 mt-2">
            Consistent attendance this week! Keep it up.
          </p>
        </div>
      ),
    },
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-1">
              {[1, 1, 1, 1, 0].map((v, i) => (
                <div
                  key={i}
                  className={`w-2 h-2 rounded-full ${v ? "bg-red-500" : "bg-gray-200"}`}
                ></div>
              ))}
            </div>
          </div>
    ...
    const fetchSchedule = async () => {
      try {
        const response = await fetch(
          "http://localhost:5000/api/student/schedule?studentId=3",
        );
        if (response.ok) {
    ...
    const fetchAnnouncements = async () => {
      try {
        ...
        if (response.ok) {
          ...
          // Filter by enrolment
          const filtered = data.filter((ann: Announcement) =>
            enrolledClassIds.includes(ann.classId || ""),
          );
    ...
    <div className="space-y-10 pb-12">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-2 border-b border-gray-100">
        <div>
          ...
          <p className="text-gray-500 font-medium ml-5">
            Welcome back,{" "}
            <span className="text-blue-600 font-bold">Emma Wilson</span>! You're
            doing great.
          </p>
        </div>
        <div className="flex items-center gap-3 bg-white px-5 py-3 rounded-2xl border border-gray-100 shadow-sm">
          <div className="flex -space-x-2">
            ...
            <div className="w-8 h-8 rounded-full bg-purple-100 border-2 border-white flex items-center justify-center text-[10px] font-black text-purple-600">
              W
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        <div className="lg:col-span-1 space-y-8">
          {/* Schedule Section */}
          <div className="bg-white p-6 rounded-4xl border border-gray-100 shadow-xl shadow-blue-500/5 relative overflow-hidden">
            ...
            <div className="space-y-4">
              {todayClasses.length > 0 ? (
                todayClasses.map((cls: TodayClass, idx) => (
                  <div
                    key={idx}
                    className={`p-4 rounded-2xl border transition-all duration-300 flex items-center gap-4 ${
                      cls.active
                        ? "bg-blue-600 border-blue-500 shadow-lg shadow-blue-500/20"
                        : "bg-gray-50 border-gray-50 hover:bg-white hover:shadow-md"
                    }`}
                  >
                    <div
                      className={`w-12 h-12 rounded-xl font-black text-center shrink-0 flex flex-col justify-center ${
                        cls.active
                    ...
                    >
                      ...
                      <p className="text-[8px] uppercase tracking-tighter mt-0.5">
                        {cls.time.split(" ")[1]}
                      </p>
                    </div>
                    <div className="flex-1 min-w-0">
                      <h4
                        className={`text-xs font-black truncate ${cls.active ? "text-white" : "text-gray-900"}`}
                      >
                        {cls.subject}
                      </h4>
                      <p
                        className={`text-[9px] font-bold ${cls.active ? "text-blue-100" : "text-gray-400"}`}
                      >
        ...
        <div className="lg:col-span-3 space-y-10">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="bg-white rounded-4xl shadow-sm border border-gray-100 p-8 relative overflow-hidden group h-full">
                ...
                {[
                  ...
                  <div key={i} className="group cursor-pointer">
                    <div className="flex items-start gap-3 p-3 bg-gray-50 border border-gray-50 rounded-2xl group-hover:bg-white group-hover:shadow-lg transition-all">
                      <div
                        className={`w-1 h-6 rounded-full bg-${d.color}-500 shrink-0`}
                      ></div>
                      <div className="min-w-0">
                        <p className="font-bold text-gray-900 text-[13px] mb-0.5 leading-tight truncate">
                          {d.title}
                        </p>
                        <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">
                          {d.due}
            ...
            <div className="bg-white rounded-4xl shadow-sm border border-gray-100 p-8 relative overflow-hidden h-full">
              <div className="absolute top-0 left-0 w-full h-1 bg-linear-to-r from-blue-500 via-purple-500 to-amber-500"></div>
              ...
              <div className="space-y-5">
                {announcements.length > 0 ? (
                  announcements.map((ann, idx) => {
                    const sourceClass = todayClasses.find(
                      (c: TodayClass) => c.classId === ann.classId,
                    );
                    return (
                      <div
                        key={idx}
                        className="relative pl-6 border-l border-gray-100 group cursor-pointer hover:border-blue-600 transition-colors"
                      >
                        ...
                        <h4 className="text-xs font-bold mb-1 text-gray-900 leading-normal group-hover:text-blue-600 transition-colors">
                          {ann.title}
                        </h4>
                        <p className="text-[11px] text-gray-500 line-clamp-2 leading-relaxed opacity-70">
                          {ann.content}
                        </p>
                      </div>
                    );
          ...
          <div className="bg-white p-8 rounded-3xl border border-gray-100 shadow-xl shadow-purple-500/5 overflow-hidden">
            <div className="flex items-center justify-between mb-8">
              <div>
                ...
                <p className="text-sm text-gray-500 font-medium tracking-tight opacity-70">
                  Your most recent grades across topics
                </p>
              </div>
            </div>
